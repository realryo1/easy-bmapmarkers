...
- 1行目が設定済みプレフィックス/サフィックス形式で無効名の場合はデフォルト値 `map` が使用されます。
- タグ内部にスペースは入れないでください（デフォルト設定時: 有効 `=map=` / 無効 `= map =`）。
...
- **3行目**: 順番（整数、例: `1`, `2`, `3`）
- **4行目**: 任意の色コード（`#RRGGBB` または `#RRGGBBAA`）
...
- 先頭点が `=BMLineUnder=`（デフォルト設定時）の場合、その line 全体が「地形に隠れない」表示になります。
- 先頭点が `=BMLine=`（デフォルト設定時）の場合、通常表示になります。
- 色も先頭点（最小 order）で決まります。
- 先頭点の4行目が `#RRGGBB` の場合、その RGB が line 全体に適用されます（透過率は既定値）。
- 先頭点の4行目が `#RRGGBBAA` の場合、RGBA（透過率含む）で line 全体に適用されます。
- 色コードが不正な場合は既定色にフォールバックします。
...
- `=icon=` / `=BMLine=` / `=BMLineUnder=`（デフォルト設定時）の登録はイベント駆動（看板操作）で動作し、コマンド不要です。
...
- 値はそのまま看板1行目で使えます（デフォルト設定時の例: `=bank=`, `=star=`）。
